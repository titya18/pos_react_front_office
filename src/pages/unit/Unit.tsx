...
const Unit: React.FC = () => {
    ...
    const [selectUnit, setSelectUnit] = useState<{ id: number | undefined, name: string, type: string } | null>(null);
    ...
    const fetchUnits = async () => {
        setIsLoading(true);
        try {
            const { data, total } = await apiClient.getAllUnitsWithPagination(
                sortField,
                sortOrder,
                page,
                search,
    ...
    const exportData = units.map((unit, index) => ({
        ...
        "Name": unit.name,
        "Type": unit.type,
        "Created At": unit.createdAt ? dayjs.tz(unit.createdAt, "Asia/Phnom_Penh").format("DD / MMM / YYYY HH:mm:ss") : '',
        "Created By": `${unit.creator?.lastName || ''} ${unit.creator?.firstName || ''}`,
        "Updated At": unit.updatedAt ? dayjs.tz(unit.updatedAt, "Asia/Phnom_Penh").format("DD / MMM / YYYY HH:mm:ss") : '',
        "Updated By": `${unit.updater?.lastName || ''} ${unit.updater?.firstName || ''}`,
    }));

    const totalPages = Math.max(1, Math.ceil(total / pageSize));

    const QueryClient = useQueryClient();
    const handleAddorEditUnit = async (id: number | null, name: string, type: string) => {
        try {
    ...
    const handleEditClick = (unitData: UnitType) => {
        setSelectUnit({
            id: unitData.id,
            name: unitData.name,
            type: unitData.type
                        ...
                        <div className="relative">
                            ...
                            <div className="dataTable-wrapper dataTable-loading no-footer sortable searchable">
                                <div className="dataTable-top">
                                    ...
                                    />
                                    <ExportDropdown data={exportData} prefix="units" />
                                </div>
