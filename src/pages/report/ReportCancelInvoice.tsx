...
const columns = [
    ...
    "Status", "Grand Total", "Paid", "Due", "Canceled At", "Canceled By",
    "Reason", "Created At", "Created By", "Updated At", "Updated By", "Note/Print"
];
...
const ReportCancelInvoice: React.FC = () => {
    ...
    const fetchInvoices = useCallback(async () => {
        ...
        try {
            ...
            const { data, total: totalResult, summary } = await apiClient.getAllCancelReportInvoices(params);
            setInvoiceData(data || []);
    ...
    const exportData = invoiceData.map((inv, index) => ({
        ...
        "Canceled At": inv.deletedAt ? dayjs.tz(inv.deletedAt, "Asia/Phnom_Penh").format("DD / MMM / YYYY HH:mm:ss") : 'N/A',
        "Canceled By": `${inv.deleter?.lastName || ''} ${inv.deleter?.firstName || ''}`,
                                        ...
                                        <table id="myTable1" className="whitespace-nowrap dataTable-table">
                                            ...
                                            <tbody>
                                                {invoiceData && invoiceData.length > 0 ? (
                                                    ...
                                                    <tr>
                                                        <td colSpan={3}>No Cancel Invoice Found!</td>
                                                    </tr>
                                                )}
                                            </tbody>
                                        </table>
