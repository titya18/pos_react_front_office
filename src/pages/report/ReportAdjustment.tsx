...
const ReportAdjustment: React.FC = () => {
    ...
    const fetchAdjustments = useCallback(async () => {
        ...
        try {
            const params = {
                ...
                adjustType: (adjustType !== "ALL" ? adjustType : undefined) as "POSITIVE" | "NEGATIVE" | undefined,
    ...
    const exportData = adjustmentData.map((adjust, index) => ({
        ...
        "Total QTY": adjust.totalQuantity,
        "Approved At": adjust.approvedAt ? dayjs.tz(adjust.approvedAt, "Asia/Phnom_Penh").format("DD / MMM / YYYY HH:mm:ss") : 'N/A',
        "Approved By": adjust.approvedAt ? `${adjust.approver?.lastName || ''} ${adjust.approver?.firstName || 'N/A'}` : 'N/A',
        "Cancelled AT": adjust.deletedAt ? dayjs.tz(adjust.deletedAt, "Asia/Phnom_Penh").format("DD / MMM / YYYY HH:mm:ss") : '',
        "Cancelled By": adjust.deletedAt ? `${adjust.deleter?.lastName || ''} ${adjust.deleter?.firstName || ''}` : '',
        "Cancelled Reason": adjust.delReason || '',
                        ...
                        <div className="relative">
                            ...
                            <div className="flex gap-2 mb-4 flex-wrap items-end">
                                ...
                                <div>
                                    ...
                                    <select value={adjustType} onChange={(e) => updateParams({ adjustType: e.target.value, page: 1 })} className="form-select">
                                        ...
                                        <table id="myTable1" className="whitespace-nowrap dataTable-table">
                                            ...
                                            <tbody>
                                                {adjustmentData && adjustmentData.length > 0 ? (
                                                    ...
                                                    <tr>
                                                        <td colSpan={3}>No Stock Adjustment Report Found!</td>
                                                    </tr>
                                                )}
                                            </tbody>
                                        </table>
