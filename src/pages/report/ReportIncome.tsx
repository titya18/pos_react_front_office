...
const ReportIncome: React.FC = () => {
    ...
    const fetchIncomes = useCallback(async () => {
        ...
        try {
            ...
            const { data, total: totalResult } = await apiClient.getAllReportIncomes(params);
            setIncomes(data || []);
    ...
    useEffect(() => {
        fetchBranches();
        fetchIncomes();
    }, [fetchBranches, fetchIncomes]);

    // Filter handler
    const handleClearAllFilter = () => {
        navigate("/reportIncome");
    };
    ...
    const incomeData = incomes.map((inc, index) => ({
        ...
        "Cancelled At": inc.deletedAt ? dayjs.tz(inc.deletedAt, "Asia/Phnom_Penh").format("DD / MMM / YYYY HH:mm:ss") : '',
        "Cancelled By": inc.deletedAt ? `${inc.deleter?.lastName || ''} ${inc.deleter?.firstName || ''}` : '',
        "Cancelled Reason": inc.delReason || ''
    }));
                        ...
                        <div className="relative">
                            ...
                            <div className="dataTable-wrapper dataTable-loading no-footer sortable searchable">
                                <div className="dataTable-top">
                                    ...
                                    />
                                    <ExportDropdown data={incomeData} prefix="Income_Report" />
                                </div>
