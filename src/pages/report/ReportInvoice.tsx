...
const ReportInvoice: React.FC = () => {
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      ...
      const { data, total: totalResult, summary } =
        await apiClient.getAllReportInvoices(params);
  ...
  const exportData = invoiceData.map((inv, index) => ({
    ...
    "Invoice Date": inv.orderDate
      ? format(new Date(inv.orderDate), "dd-MMM-yyyy")
      : "",
    Reference: inv.ref,
    ...
    Profit: Number((inv as any).orderProfit || 0),
    "Approved At": inv.approvedAt
      ? dayjs.tz(inv.approvedAt, "Asia/Phnom_Penh").format("DD / MMM / YYYY HH:mm:ss")
      : "N/A",
    "Approved By": `${inv.approver?.lastName || ""} ${inv.approver?.firstName || ""}`.trim(),
                      ...
                      <tbody>
                        {invoiceData.length > 0 ? (
                          invoiceData.map((rows, index) => {
                            ...
                            const orderProfit = Number((rows as any).orderProfit || 0);
                              ...
                              <tr key={rows.id || index}>
                                ...
                                {visibleCols.includes("Approved At") && (
                                  <td>
                                    {rows.approvedAt
                                      ? dayjs
                                          .tz(rows.approvedAt, "Asia/Phnom_Penh")
                                ...
                                {visibleCols.includes("Approved By") && (
                                  <td>
                                    {rows.approvedAt
                                      ? `${rows.approver?.lastName || ""} ${
                                          rows.approver?.firstName || ""
                                        }`
                                      : "N/A"}
                                  </td>
                                )}
