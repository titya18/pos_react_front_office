...
import { faArrowUpZA, faArrowDownAZ, faPrint, faClose, faSave, faFileInvoice, faDollarSign, faCircleCheck, faClockRotateLeft } from '@fortawesome/free-solid-svg-icons';
...
const ReportPurchase: React.FC = () => {
    ...
    }>({ totalPurchase: 0, grandTotalAmount: 0, totalPaidAmount: 0, totalRemainAmount: 0 });
    ...
    const fetchQuotations = useCallback(async () => {
        ...
        try {
            ...
            const { data, total: totalResult, summary } = await apiClient.getAllReportPurchases(params);
                                                    ...
                                                    purchaseData.map((rows, index) => (
                                                        <tr key={index}>
                                                            ...
                                                            {visibleCols.includes("Updated At") && (
                                                                <td>{rows.updatedAt ? dayjs.tz(rows.updatedAt, "Asia/Phnom_Penh").format("DD / MMM / YYYY HH:mm:ss") : "N/A"}</td>
                                                            )}
