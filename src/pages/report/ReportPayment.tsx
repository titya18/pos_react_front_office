...
const ReportPaymentInvoice: React.FC = () => {
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      ...
      const { data, total: totalResult, summary } =
        await apiClient.getAllPaymentReportInvoices(params);
  ...
  const exportData = invoiceData.map((inv, index) => ({
    ...
    "Amount Paid": Number(inv.totalPaid || 0),
                        ...
                        {invoiceData.length > 0 ? (
                          invoiceData.map((rows, index) => (
                            <tr key={rows.id || index}>
                              ...
                              {visibleCols.includes("Invoice's ID") && (
                              ...
                              {visibleCols.includes("Amount Paid") && (
                                <td style={{ color: "green" }}>
                                  $ {formatCurrency(Number(rows.totalPaid || 0))}
                                </td>
                              )}
